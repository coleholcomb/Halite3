...
class GameMap:
    ...
    def get_one_move_direction(self, source, target):

        dist =  self.calculate_distance(source, target)
        if dist > 1:
            raise ValueError
        elif dist == 0:
            return (0,0)
        ...
        if np.abs(direction[0]) > 1:
            direction = (-1*np.sign(diffx), 0)
        elif np.abs(direction[1]) > 1:
            direction = (0, -1*np.sign(diffy))
    ...
    def naive_navigate_end(self, ship, destination):
        """
        Returns a singular safe move towards the destination.
